...
class Reviewer:
    ...
    def save_to_sql(self):
        ...
        sql = """INSERT INTO Users (user_name, country, rev_wrote) VALUES ("%s" , "%s" , %d)""" % (name, "UK", self.reviews)
        my_cursor.execute(sql)
        my_db.commit()

        content = re.sub(r'\W+', ' ', self.content)
        sql = """INSERT INTO Reviews (user_id, company_id, title , content) VALUES (%d, %d, "%s", "%s")""" % (112233, 445566, 'my_title', content[:250])
        my_cursor.execute(sql)
        my_db.commit()


        sql = """INSERT INTO Companies (name , website, rating, num_reviews) VALUES ("%s" , "%s" , %d, %d)""" % (self.name, "www.mywebsite.com", self.stars, 1)
        my_cursor.execute(sql)
        my_db.commit()
