...
logging.basicConfig(filename='log_scrapper.log', level=logging.DEBUG, format='%(asctime)s:%(funcName)s:%(message)s')
...
def check_status():
    ...
    logging.info('r status code ' , r.status_code)
    logging.info('is status code correct ' , r.status_code == requests.codes.ok)
    logging.info('request redirects' , requests.codes['temporary_redirect'])
    logging.info('request codes. teapot' , requests.codes.teapot)
    logging.info('request codes ' , requests.codes['o/'])
    pass
...
def scrap(company, num_pages):
    ...
    website = []
    urls = []
    logging.info('the website we being scrapped from' , 'https://www.trustpilot.com/review/' + company)

    for p in range(1, int(num_pages)):

        page_url = requests.get('https://www.trustpilot.com/review/' + company + '?page=' + str(p))
        logging.info('the website we being scrapped from')
        ...
        for review in review_card:
            try:
                ...
                logging.critical('exception in scrapping data given as ' , e)
...
def export_csv(company, num_pages):
    ...
    try:
        ...
        logging.CRITICAL('error in creating the csv file')
    else:
...
def export_sql(company, num_pages):
    ...
    try:
        connection = pymysql.connect(host='localhost',
                                     user='root',
                                     password= r"""`7fnbQ3F2r\N'QM(""",
                                     database = 'trustpilot',
                                     charset='utf8mb4',
                                     cursorclass=pymysql.cursors.DictCursor)
        print(f'Database exists. Succesfully connected')
        logging.info(f'Database exists. Succesfully connected')
    ...
    for i in range(len(reviews_dict['ratings'])):
        #print(reviews_dict['ratings'][i])
        reviews_infos = [reviews_dict['ratings'][i], reviews_dict['titles'][i], reviews_dict['contents'][i],reviews_dict['replies'][i]]
        users_infos = [users_dict['names'][i], users_dict['countries'][i], users_dict['rev_wrote'][i]]
        companies_infos = [companies_dict['company_names'][0], companies_dict['company_ratings'][0],companies_dict['website'][0], companies_dict['num_reviews'][0]]
        reviews_insert_query = 'INSERT INTO Reviews(rating,title,content,replies) VALUES (%s,%s,%s,%s)'
        c.execute(reviews_insert_query, (reviews_dict['ratings'][i],reviews_dict['titles'][i], reviews_dict['contents'][i],
                            reviews_dict['replies'][i]))
        users_insert_query = 'INSERT INTO Users(user_name,country,rev_wrote) VALUES (%s,%s,%s)'
        c.execute(users_insert_query, (users_dict['names'][i], users_dict['countries'][i], users_dict['rev_wrote'][i]))
    #c.commit()
